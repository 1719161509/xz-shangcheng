...
router.post('/reg',(req,res)=>{
    ...
    pool.query('INSERT INTO hd_user SET ?',[obj],(err,result)=>{
        if(err) throw err;
        console.log(result);
...
router.post('/login',(req,res)=>{
   ...
   //执行SQL语句
   pool.query('SELECT * FROM hd_user WHERE uname=? AND upwd=?',[obj.uname,obj.upwd],(err,result)=>{
       if(err)  throw err;
       console.log(result);
...
router.get('/detail',(req,res)=>{
   ...
   //执行SQL语句
   pool.query('SELECT * FROM hd_user WHERE uid=?',[obj.uid],(err,result)=>{
    if(err)  throw err;
    console.log(result);
...
router.get('/update',(req,res)=>{
   ...
   //执行SQL语句
   pool.query('UPDATE hd_user SET ? WHERE uid=?',[obj,obj.uid],(err,result)=>{
       if(err) throw err;
       if(result.affectedRows>0){
...
router.get('/list',(req,res)=>{
    ...
    //执行查询
    pool.query('SELECT * FROM hd_user limit ?,?',[start,obj.count],(err,result)=>{
        if(err)  throw err;
        console.log(result);
...
router.get('/delete',(req,res)=>{
    ...
    //执行sql语句
    pool.query('DELETE FROM hd_user WHERE uid=?',[obj.uid],(err,result)=>{
        if (err) throw err;
        console.log(result);
        if(result.affectedRows>0){
            res.send({
